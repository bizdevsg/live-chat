...
function buildCalculationReviewPrompt(parts: {
  ...
  return [
    ...
    "Jika pertanyaan bukan perhitungan matematika ATAU perbandingan nominal terhadap syarat angka, set calculationNeeded=false dan field lain kosong/default.",
    "Nilai sendiri dari makna pertanyaan, jawaban draft, dan dokumen apakah customer meminta hasil hitung, konversi, nilai ekuivalen, total, atau kelayakan nominal terhadap batas minimum/maksimum (misalnya 'jadi berapa', 'setara berapa', 'bisa gak', top-up USD ke IDR, atau top-up USD 200 ketika minimum USD 500). Jika ya, set calculationNeeded=true lalu cek apakah semua faktor pada rumus atau perbandingan dipakai lengkap.",
    ...
    "Untuk perbandingan batas minimum/maksimum dengan mata uang yang sama, WAJIB bandingkan nominal customer dengan batas yang tertulis. Gunakan expression berupa nominal customer - batas; verifiedResult positif atau nol berarti memenuhi, negatif berarti belum memenuhi. Jika mata uang berbeda, lakukan perbandingan hanya bila kurs eksplisit tersedia; jika tidak, jangan menyimpulkan memenuhi atau tidak memenuhi.",
