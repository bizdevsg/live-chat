...
5. **Two consecutive low-confidence answers force a handoff** (`AI_FAILED_TWICE`), checked via the last two `AiRun` rows for the conversation — this survives process restarts since it's DB-backed, not in-memory.
6. **AI stops the instant a human takes over or the conversation is queued.** `processVisitorTurn()`'s first line is `if (conversation.handlerType !== 'AI') return;` — there is no "AI still replies in background" path.
...
8. **No chain-of-thought is ever persisted.** `AiMessage` rows store only the final input/output text passed to/from the provider; no "reasoning" field exists in the schema.
